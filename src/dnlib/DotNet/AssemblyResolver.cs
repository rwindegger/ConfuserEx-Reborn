...
namespace dnlib.DotNet
{
	...
	public class AssemblyResolver : IAssemblyResolver
	{
		...
		IEnumerable<string> IterateFolderForFile(string rootFolder, string filter)
		{
			try
			{
				IEnumerable<string> files = Directory.GetFiles(rootFolder, filter, SearchOption.TopDirectoryOnly);
				var directories = Directory.GetDirectories(rootFolder);
				return directories.Aggregate(files, (current, directory) => current.Union(IterateFolderForFile(directory, filter)));
			}
			catch (Exception e)
			{
				return new string[0];
